...
# ---------------------------------------------------------------------------
SESSION_DIR       = r"C:\Users\wanglab\Desktop\Club Like Endings\102225_1"
...
# Set conservatively to ~2 frame durations at 500 fps (4 ms).
VIDEO_FPS         = 500
MAX_FRAME_GAP_S   = 2.0 / VIDEO_FPS   # 4 ms
...
def compute_spike_rate_modulation(histograms: dict) -> dict:
    """
    For each unit, compute spike rate change due to phase:
        delta = max(bin_counts) - min(bin_counts)

    Returns dict {unit_id: delta}
    """
    modulations = {}
    for unit, counts in histograms.items():
        if counts.sum() == 0:
            continue
        modulations[unit] = float(counts.max() - counts.min())

    return modulations


def plot_cumulative_fraction(modulations: dict, out_path: str):
    """
    Plot cumulative fraction of neurons vs spike rate change due to phase (Hz).

    X-axis : spike rate modulation depth  Δrate = max − min  (Hz)
    Y-axis : cumulative fraction of neurons  [0, 1]
    """
    FONT       = "Arial"
    FONT_SZ    = 13
    FONT_SZ_LBL = 14
    ...
    if len(modulations) == 0:
        print("  WARNING: No units with spikes — skipping cumulative plot.")
        return

    values = np.sort(list(modulations.values()))
    n      = len(values)
    cum_frac = np.arange(1, n + 1) / n

    # Extend to (0, 0) so the curve starts at the origin
    x = np.concatenate([[0], values])
    y = np.concatenate([[0], cum_frac])

    fig, ax = plt.subplots(figsize=(6, 5))

    ax.step(x, y, where="post", color="black", linewidth=2)
    ax.scatter(values, cum_frac, color="black", s=30, zorder=5)

    ax.set_xlabel("Spike rate change due to phase (Hz)", fontsize=FONT_SZ_LBL,
                  fontfamily=FONT)
    ax.set_ylabel("Cumulative fraction of neurons", fontsize=FONT_SZ_LBL,
                  fontfamily=FONT)
    ax.set_title("Phase Modulation Depth — All Units", fontsize=FONT_SZ_LBL + 1,
                 fontfamily=FONT, fontweight="bold")
    ...
    ax.annotate(f"n = {n} units", xy=(0.97, 0.05), xycoords="axes fraction",
                ha="right", fontsize=FONT_SZ - 2, fontfamily=FONT)
...
if __name__ == "__main__":
    ...
    print("\n=== STEP 7: Cumulative fraction plot ===")
    modulations = compute_spike_rate_modulation(histograms)
    plot_cumulative_fraction(
        modulations,
        os.path.join(OUTPUT_DIR, "cumulative_phase_modulation.png")
    )
