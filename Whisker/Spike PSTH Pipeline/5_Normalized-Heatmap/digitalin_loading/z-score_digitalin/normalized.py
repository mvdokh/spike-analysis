"""
Z-Score Normalized PSTH Heatmap Utilities (Digital Input Version)

This module provides utilities for creating Z-score normalized PSTH heatmaps using
...
def create_normalized_psth_heatmap(spikes_file: str, digitalin_file: str, 
                                  ...
                                  pico_channel: int = 0) -> Tuple[plt.Figure, np.ndarray, List[int], np.ndarray]:
    """
    Create a Z-score normalized PSTH heatmap for multiple units using digitalin.dat file.
...
def create_multiple_duration_normalized_heatmaps(spikes_file: str, digitalin_file: str,
                                               ...
                                               post_interval_ms: float = 10, smooth_window: Optional[int] = 5,
                                               exclude_whisking: bool = False, whisking_file: Optional[str] = None,
                                               save_dir: Optional[str] = None, sampling_rate: int = 30000, 
                                               pico_channel: int = 0) -> Dict[float, Tuple]:
    """
    Create Z-score normalized PSTH heatmaps for multiple interval durations using digitalin.dat file.
...
def compare_raw_vs_normalized(spikes_file: str, digitalin_file: str, duration_ms: float,
                             ...
                             sampling_rate: int = 30000, pico_channel: int = 0):
    """
