...
def load_data(spikes_file: str, intervals_file: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Load spike and interval data from CSV files (legacy function for backward compatibility).
    ...
    """
    return load_data_with_digitalin(spikes_file=spikes_file, intervals_file=intervals_file, use_digitalin=False)
...
def create_psth_heatmap(spikes_file: str = None, intervals_file: str = None, digitalin_file: str = None,
                       duration_ms: float = 25, units: Optional[List[int]] = None,
                       bin_size_ms: float = 0.6, pre_interval_ms: float = 5, 
                       post_interval_ms: float = 10, smooth_window: Optional[int] = 5,
                       sampling_rate: int = 30000, use_digitalin: bool = False,
    ...
    Args:
        spikes_file: Path to spikes.csv file
        intervals_file: Path to pico_time_adjust.csv file (fallback if not using digitalin)
        ...
        post_interval_ms: Time after interval in ms
        smooth_window: Number of bins for smoothing (optional)
        sampling_rate: Sampling rate for digitalin.dat processing
        use_digitalin: Whether to use digitalin.dat (True) or CSV (False)
...
def create_multiple_duration_heatmaps(spikes_file: str = None, intervals_file: str = None, digitalin_file: str = None,
                                    durations_ms: List[float] = [5, 10, 25], units: Optional[List[int]] = None,
                                    bin_size_ms: float = 0.6, pre_interval_ms: float = 5,
                                    post_interval_ms: float = 10, smooth_window: Optional[int] = 5,
                                    sampling_rate: int = 30000, use_digitalin: bool = False,
