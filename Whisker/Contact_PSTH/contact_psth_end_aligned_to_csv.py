"""
Contact PSTH to CSV (End-Aligned) — Export per-millisecond firing rates
aligned to contact offset (interval end)

Identical to contact_psth_to_csv.py but spikes are referenced to the
interval END rather than the START.  t = 0 marks the moment the whisker
breaks contact.
...
Output is saved to:
    <data_dir>/contact_psth_end_aligned_csv_output/
              contact_psth_end_aligned_firing_rates.csv

Usage
-----
    python contact_psth_end_aligned_to_csv.py --data_dir <session_dir>
                                              [--contact_dir <dir>]
                                              [--output_dir <dir>]
...
from contact_psth_end_aligned import align_spikes_to_ends
...
def main():
    parser = argparse.ArgumentParser(
        description="Export end-aligned PSTH firing rates (per ms bin) to "
                    "CSV for each unit and contact-interval file. "
                    "t = 0 is the contact offset (interval end).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python contact_psth_end_aligned_to_csv.py ^
