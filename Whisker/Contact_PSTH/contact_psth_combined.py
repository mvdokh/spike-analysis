"""
Contact PSTH Combined — One image per unit with all interval CSVs

For each unit, produces a single tall figure where each row is a
(PSTH, Raster) pair for one contact-interval CSV file.  This makes it
easy to compare responses across whiskers / contact types at a glance.

Reuses data-loading and alignment functions from contact_psth.py.

Usage
-----
    python contact_psth_combined.py --data_dir <session_dir>
                                    [--contact_dir <dir>]
                                    [--output_dir <dir>]
...
def draw_psth_raster_on_axes(ax_psth, ax_raster, trials, label,
    ...
    """
    Draw a PSTH + raster pair onto the provided axes.
    """
    if not trials:
...
def run_combined(data_dir, contact_dir=None, output_dir=None,
    ...
    if output_dir is None:
        output_dir = os.path.join(data_dir, "contact_psth_combined_output")
    ...
    # Discover contact CSVs; put contact_intervals.csv first (top center)
    ...
    # Build ordered list: contact_intervals first, then the rest
    contact_files = []
    ...
    csv_basenames = [os.path.splitext(os.path.basename(f))[0] for f in contact_files]
    ...
    # Layout: first CSV centered alone in row 0, remaining CSVs in rows of 3
    ...
    for unit in process_units:
        ...
        body_start = 1 if has_top else 0
        for i in range(n_remaining):
            csv_idx = body_start + i  # index into contact_files / csv_events
            # but csv_idx in the ordered list is (1 if has_top else 0) + i
        ...
        fig.suptitle(f"Unit {unit}", fontsize=14, fontweight="bold",
                     y=0.995, va="top")

        out_name = f"unit_{unit}_combined.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate combined PSTH/raster images — one per unit, "
                    "all contact CSVs stacked vertically.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python contact_psth_combined.py ^
      --data_dir "C:\\Users\\wanglab\\Desktop\\Club Like Endings\\102525_1"
        """,
    )
