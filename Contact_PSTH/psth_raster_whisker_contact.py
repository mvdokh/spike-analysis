...
Time alignment: spike time (s) is multiplied by FPS (350) to get spike time in frame
...
def main():
    base = Path("/Users/martindokholyan/Desktop/Plots/4")
    parser = argparse.ArgumentParser(description="PSTH + raster per unit, aligned to whisker contact onset.")
    parser.add_argument("interval_csv", nargs="?", default=str(base / "interval_4_whisker_contact.csv"),
    ...
    for unit in units:
        ...
        rel_times, trial_inds = raster_for_unit(spike_frames, spike_units, intervals, unit)
        ...
        # Raster
        if len(rel_times) > 0:
            ax_raster.scatter(rel_times, trial_inds, marker="|", s=20, c="black", linewidths=0.8)
