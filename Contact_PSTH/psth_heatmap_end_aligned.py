"""
Z-Score Normalised End-Aligned PSTH Heatmap
============================================

Same as psth_heatmap.py but for end-aligned (contact offset) data.

For each interval, produces a heatmap where:
  - Each row is a unit
  - Each column is a 1-ms time bin (t = 0 is contact offset)
  - Values are z-scored per unit (baseline = [−100, −30) ms)
  - Units are sorted by peak response latency in [0, 30) ms
  - A vertical dashed line marks contact offset (t = 0 ms)

Outputs one PNG per interval into
``<session>/contact_psth_end_aligned_csv_output/heatmaps/``.

Usage
-----
    python psth_heatmap_end_aligned.py --data_dir "C:\\path\\to\\session"
    python psth_heatmap_end_aligned.py --csv "path/to/end_aligned_firing_rates.csv"
"""
...
def make_heatmap(df, interval, out_dir, vmin=-3, vmax=5):
    ...
    where baseline = [−100, −30) ms (well before contact offset).
    """
    ...
    # Sort rows by peak response latency in [0, 30) ms window
    bins_arr = np.array(bins)
    post_mask = (bins_arr >= 0) & (bins_arr < 30)
    if post_mask.sum() > 0:
    ...
    ax.set_xlabel("Time from contact offset (ms)", fontsize=11)
    ...
    ax.set_title(f"{title}  (n = {n_trials} events)\n"
                 f"Z-score normalised firing rate — end-aligned",
    ...
    fig.tight_layout()
    fname = f"heatmap_end_{interval}.png"
...
def run(csv_path, out_dir=None):
    ...
    print(f"\nGenerating {len(intervals)} end-aligned heatmaps …")
...
def main():
    parser = argparse.ArgumentParser(
        description="Z-score normalised end-aligned PSTH heatmap per interval.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    g = parser.add_mutually_exclusive_group(required=True)
    g.add_argument("--data_dir",
                   help="Session directory (looks for "
                        "contact_psth_end_aligned_csv_output/)")
    g.add_argument("--csv",
                   help="Direct path to end-aligned firing-rates CSV")
